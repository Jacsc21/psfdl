...
interpret (("color":xs):yss)  start rules m depth len _   width = interpret yss start rules m depth len (interpretCode xs) width
interpret (("width":arg:xs):yss)  start rules m depth len rgb _     = interpret yss start rules m depth len rgb (read arg)

interpretColor :: [] -> RGB
interpretColor xs 
 | length xs == 1 = RGB interpretString xs 
 | otherwise RGB = interpretCode xs

interpretString :: [] -> RGB
interpretString color
 | color == "Blue" = RGB 0 0 255

interpretCode :: [] -> RGB
interpretCode xs = RGB (xs !! 0) (xs !! 1) (xs !! 2)
