...
drawIt (w, x, y, angle, len, rgb, width) (Forward:xs)          = let
  x' = x+len*(cos angle)
  y' = y-len*(sin angle)
  widthInt = round width
  rgbPen = rgb
  in do
    p <- createPen Solid widthInt rgbPen
...
interpret (("color":xs):yss)  start rules m depth len _   width = interpret yss start rules m depth len (interpretColor xs rgb) width where
  interpretColor [c, "Black"]       rgb = \x -> if x == (head c) then (RGB 0 0 0) else rgb x
  interpretColor [c, "Red"]         rgb = \x -> if x == (head c) then (RGB 255 0 0) else rgb x
  interpretColor [c, "Green"]       rgb = \x -> if x == (head c) then (RGB 0 255 0) else rgb x
  interpretColor [c, "Blue"]        rgb = \x -> if x == (head c) then (RGB 0 0 255) else rgb x
  interpretColor [c, "White"]       rgb = \x -> if x == (head c) then (RGB 255 255 255) else rgb x
  interpretColor [c, "Cyan"]        rgb = \x -> if x == (head c) then (RGB 0 255 255) else rgb x
  interpretColor [c, "Magenta"]     rgb = \x -> if x == (head c) then (RGB 255 0 255) else rgb x
  interpretColor [c, "Yellow"]      rgb = \x -> if x == (head c) then (RGB 255 255 0) else rgb x
  interpretColor [c, "Random"]      rgb = \x -> if x == (head c) then (RGB 48 213 200) else rgb x
  interpretColor [c, (Int)]         rgb = \x -> if x == (head c) then (RGB(read (xs !! 0)) (read (xs !! 1)) (read (xs !! 2))) else rgb x
interpret (("width":arg:xs):yss)  start rules m depth len rgb _     = interpret yss start rules m depth len rgb (read arg)


--RGB(read (xs !! 0)) (read (xs !! 1)) (read (xs !! 2))

-- read from given file and return a fractal
readFractal fileName = do
  text <- readFile fileName
  let rgb = RGB 0 0 0
  return (interpret (split text [] []) [] [] (\x -> error "unknown command") 0 0 rgb 0)
